...
class prep_tools:
    def latex_to_df(x, cols, show = True, O_names = False, use_int = False):
        """
...
class pca_calc:
    ...
    def proj(V, x, components = None, V_full = False):
        ...
        print("The projected coordinates on component {} are: {}".format(components, proj_coord))
        return proj_coord

class cross_val:
    def trained_models(k1, k2, S, show_res = True):
        """
        ...
        if show_res:
            print("A total of {} models were trained, as {} models were trained {} times each.".format(runs, S, pr_model))
...
class decision_trees:
    ...
    def purity_gain(root, v1, v2, purity_measure, accuracy = False):
        ...
        if purity_measure == "entropy":
            for i in range(2):
                Iv += decision_trees_exam.entropy(v[i])*sum(v[i])/sum(root)
...
class ensemble:
    ...
    def conf_matrix_stats(TP, FN, FP, TN, stats):
        """
        ...
        if "show" in stats:
            ensemble.plot_confusion_matrix(cm)
        ...
        if "all" in stats:
            all = pd.DataFrame({"Stat": ["Precision", "Recall", "Accuracy", "Error", "TPR", "FPR"], "Value":[p, r, acc, err, TPR, FPR]})
            print(all)
    
    def make_conf_matrix(true_val, pred_val, show_conf_matrix = False):
        """
        ...
        ensemble.conf_matrix_stats(TP = TP, FP = FP, FN = FN, TN = TN, stats = stats)
...
class cluster:
    ...
    def cluster_similarity(x,y):
        ...
        for i in range(N):
            for j in range(i):
                if y[i]!=y[j] and x[i]!=x[j]: f00 += 1;     # different class, different cluster    
                elif y[i]==y[j] and x[i]==x[j]: f11 += 1;   # same class, same cluster
                elif y[i]==y[j] and x[i]!=x[j]: f10 += 1;   # same class, different cluster    
...
class similarity:
    def measures(x,y):
        ...
        for i in range(N):
            ...
            if y[i]!=1 and x[i]!=1: f00 += 1;     # different class, different cluster    
            elif y[i]==1 and x[i]==1: f11 += 1;   # same class, same cluster
            elif y[i]==1 and x[i]!=1: f10 += 1;   # same class, different cluster    
            else: f01 +=1;                               # different class, same cluster
        jaccard = np.float(f11)/(f01+f10+f11)
...
class anomaly:
    def ARD(df, obs, K):
        ...
        print("The average relative density for observation O{} is {}".format(obs +1, ARD))
...
class association_mining:
    ...
    def apriori_rules(df, X, Y, sup, conf):
        """
        calculates rules 
        ----------------------
        parameters:
        ...
        frules, rules_conf_sup =  association_mining.print_apriori_rules(rules, prints=False)
